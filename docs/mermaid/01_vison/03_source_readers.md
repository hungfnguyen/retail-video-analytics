    ...
    loop Read loop (thread chạy liên tục)
        ...
        alt ret == True
            ...
            loop Reconnect with backoff
                ...
                alt ret == True
                    Note over R: reconnected! → break khỏi reconnect loop
                    R->>M: reconnect_success++
                    R->>Q: queue.put(frame)
                else ret == False
                    Note over R: attempt++, tiếp tục reconnect loop
                end
            end
            Note over R: quay về main Read loop
        end
    end
```
...
| **Thread model** | Thread riêng trong worker | Thread riêng hoặc main thread |

> **FrameQueue phải dùng `queue.Queue` (Python thread-safe).** RTSPReader thread ghi vào queue, Pipeline main thread đọc từ queue. Không dùng `list` thông thường vì không an toàn khi 2 thread truy cập đồng thời.
