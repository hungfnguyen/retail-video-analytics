...
export function VideoPanel({ frame }: VideoPanelProps) {
  const hasFrameImage = frame.image_url.trim().length > 0
  ...
  return (
    <section className="relative min-h-[490px] overflow-hidden rounded-lg border border-slate-200 bg-white shadow-[0_18px_45px_rgba(15,23,42,0.08)]">
      ...
      <div className="relative h-[440px] overflow-hidden bg-slate-200">
        {hasFrameImage ? (
          <img
            alt={`Camera ${frame.camera_id} frame ${frame.frame_id}`}
            className="absolute inset-0 h-full w-full object-cover"
            src={frame.image_url}
          />
        ) : (
          <div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(15,23,42,0.1)_1px,transparent_1px),linear-gradient(rgba(15,23,42,0.08)_1px,transparent_1px),linear-gradient(180deg,#dce7f1_0%,#cbd9e8_44%,#b7c6d3_100%)] bg-[length:58px_58px,58px_58px,100%_100%]">
        ...
        {frame.heatmap_points.map((point) => (
          <span
            className="absolute z-10 h-36 w-36 rounded-full bg-[radial-gradient(circle,rgba(250,204,21,0.6),rgba(34,197,94,0.32)_42%,rgba(37,99,235,0.16)_72%,transparent)] mix-blend-multiply"
            key={`${point.x}-${point.y}-${point.intensity}`}
            style={{
              left: `${point.x * 100}%`,
              top: `${point.y * 100}%`,
              opacity: 0.16 + point.intensity * 0.24,
              transform: `translate(-50%, -50%) scale(${0.75 + point.intensity * 0.8})`,
            }}
          />
        ))}
