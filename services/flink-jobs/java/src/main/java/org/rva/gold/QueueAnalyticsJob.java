...
public class QueueAnalyticsJob {
    ...
    public static void main(String[] args) throws Exception {
        ...
        String insertZoneMinuteMetrics = String.join("\n",
                ...
                "  FROM TABLE(TUMBLE(",
                "    TABLE rva.silver_detections_v2 /*+ OPTIONS('streaming'='true', 'monitor-interval'='1s') */,",
                "    DESCRIPTOR(capture_ts),",
                "    INTERVAL '1' MINUTE))",
                ...
                "frame_counts AS (",
                "  SELECT",
                "    store_id, camera_id, primary_zone_id, primary_zone_type, window_start, window_end, frame_index,",
                "    COUNT(*) AS frame_occupancy",
                "  FROM windowed",
                "  GROUP BY store_id, camera_id, primary_zone_id, primary_zone_type, window_start, window_end, frame_index",
                "),",
                "visitor_counts AS (",
                "  SELECT",
                "    store_id, camera_id, primary_zone_id, primary_zone_type, window_start, window_end,",
                "    COUNT(DISTINCT global_track_id) AS unique_visitors,",
                "    COUNT(*) AS detection_count",
                "  FROM windowed",
                "  GROUP BY store_id, camera_id, primary_zone_id, primary_zone_type, window_start, window_end",
                ")",
                "SELECT",
                "  f.store_id,",
                "  f.camera_id,",
                ...
                " AND f.window_end = v.window_end",
                "GROUP BY f.store_id, f.camera_id, f.primary_zone_id, f.primary_zone_type, f.window_start, f.window_end"
        );
