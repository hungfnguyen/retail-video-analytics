INSERT INTO rva_gold_serving.gold_serving_zone_daily
WITH frame_stats AS (
  SELECT
    store_id,
    camera_id,
...
visitor_stats AS (
  ...
  WHERE class_id = 0
    ...
    AND CAST(capture_ts AS DATE) BETWEEN DATE {{START_SQL}} AND DATE {{END_SQL}}
  GROUP BY store_id, camera_id, primary_zone_id, COALESCE(primary_zone_type, 'unknown'), CAST(capture_ts AS DATE)
)
SELECT
  f.store_id,
  f.camera_id,
